...
- **Fundamentals (screener.in history):** point-in-time annual 2006→2026 in `backend/data/screener_fundamentals.duckdb`. **1,305 names** (high 990 / low 302 / quarantined 10) after the 2026-06-20 674-gap scrape (660 ingested, 2 failed). **Trendlyne ∩ screener = 898** of the 1,138 live-DVM names (~98% of eligible operating companies; remaining 240 = 130 ETFs/funds + 89 financials + 21 scrape edge-cases). Classified gap list: `data/missing_screener.csv`.
- **NSE universe scope:** ~2,375 listed equities (EQUITY_L master). We hold prices for 1,505 (the liquid set; the rest are illiquid micro-caps). BSE deliberately out of scope (illiquid, NSE-keyed data sources).
- **Data-correctness check (2026-06-20):** screener-COMPUTED ROE vs Trendlyne-REPORTED ROE on the 898 overlap = Spearman 0.90, 94% within ±5pp → screener fundamentals confirmed sound; remaining DVM gaps are formula, not data.
- **Universes:** `nifty500` (504), `niftytotalmarket` (~754), `trendlyne` (the 1138 fundamentals stocks), `allnse` (full EQUITY_L, load on demand).
- **DVM validation vs Trendlyne (snapshot 2026-06-18, Spearman):** momentum **0.835**, durability **0.875** (v2 Piotroski-led, commit 65d75aa), valuation **0.445** (v1.1, commit cd4a503). Durability & momentum are at their data ceilings; valuation is data-capped ~0.44. Lifting valuation further needs NEW data (forward-PE — the export column is EMPTY — plus EV/EBITDA, dividend yield; to-do #26), not formula. Measured-and-DIDN'T-help (don't retry without new data): ROCE/D-E for durability (cross-source noise); historical-multiple percentile for valuation (orthogonal to cross-sectional cheapness, rho~0.12); every momentum reweight (current blend already best).
