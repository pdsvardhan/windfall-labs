/* ============================================================================
   Trendlyne COMPLETENESS Harvester — Windfall Labs
   ----------------------------------------------------------------------------
   Pulls the 2 NSE stocks that are >=Rs500cr on Trendlyne's own mcap but were
   absent from the dataset: SREEL (Sreeleathers), RKSWAMY (R K Swamy).
   Same all-tables pull as the megacap fix, with SPACED downloads (so Chrome's
   multi-download throttle can't drop files).

   RUN: logged-in trendlyne.com tab -> F12 -> Console -> paste -> Enter. ~1 min.
        Click "Allow" if Chrome asks about multiple downloads.

   OUTPUT (Downloads), "_completeness" suffix, merge into matching folders:
     tl_dvm_history_completeness.csv (pk,score,date,value)
     tl_<valuation_ratios|pnl_quarterly|growth_quality|ownership>_completeness.csv (pk,metric,date,value)
     tl_<pnl_annual|balance_sheet|cashflow|ratios_annual|financials_other>_completeness.csv
     tl_corporate_actions_completeness.csv / tl_result_dates_completeness.csv
     tl_shareholding_summary_completeness.csv / tl_sector_map_completeness.csv
     tl_ohlcv_completeness.csv (pk,date,open,high,low,close,last,volume)
     tl_stocks_completeness.csv (pk,nsecode,name,d_now,v_now,m_now)
============================================================================ */
(async () => {
  const SYMBOLS = ['SREEL','RKSWAMY'];
  const CONC = 2;
  ...
  box.innerHTML = '<b>Windfall · Completeness (2 stocks)</b><div id="wfp" style="margin-top:6px">starting…</div>';
  ...
  async function proc(sym){
    const r=await resolve(sym); if(!r||!r.pk)return false; const {pk,token,name}=r; let dn='',vn='',mn='';
  ...
  let idx=0, ok=0;
  async function worker(){ while(true){ const i=idx++; if(i>=SYMBOLS.length)break; log('pulling '+SYMBOLS[i]+'…'); try{ if(await proc(SYMBOLS[i]))ok++; }catch(e){} } }
  await Promise.all(Array.from({length:CONC},worker));

  log('downloading (spaced)…');
  for (const t in B){ if (B[t].length>1){ download(`tl_${t}_completeness.csv`, B[t].join('\n')+'\n'); await sleep(1500); } }
  log(`DONE · ${ok}/${SYMBOLS.length} stocks · files downloaded (spaced)`);
})();
